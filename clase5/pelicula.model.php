...
pe_fechaEstreno*/
    ...
    public function AgregarPelicula($id_genero,$id_director,$pe_nombre,$pe_duracion,$pe_fechaEstreno)
    {
        //quien te conoce sqlinjection?
        global $db;
        return $db->insert("insert into pelicula (id_genero,id_director,pe_nombre,pe_duracion,pe_fechaEstreno) values (".$id_genero.",".$id_director.",'".$pe_nombre."',".$pe_duracion.",'".$pe_fechaEstreno."')");
    ...
    {
        global $db;
        $retval = $db->select("SELECT id_genero,ge_nombre from genero");
        return $retval;
    }
        public function ListarDirectores()
    {
        global $db;
        $retval = $db->select("SELECT id_director,di_nombreArtistico from director");
        return $retval;
    }
    ...
    {
        global $db;
        $retval = $db->select('SELECT pe_nombre, ge_nombre, di_nombreArtistico, pe_duracion, GROUP_CONCAT(actor.ac_nombreArtistico SEPARATOR ", ") as actores from pelicula, pelicula_actor, director, genero, actor where pelicula_actor.id_pelicula = pelicula.id_pelicula && pelicula.id_director = director.id_director && pelicula.id_genero = genero.id_genero && pelicula_actor.id_actor = actor.id_actor group by pelicula.id_pelicula');
        return $retval;
    }
}
